...
class TestScenarios:
    ...
    def test_scenario_2b(self):
        ...
        for i, j in coords:
            ...
            assert row_count == 1, f"x0 coordinate at index {i} has {row_count} non-NaN(s); expected exactly 1"
            assert col_count == 1, f"x1 coordinate at index {j} has {col_count} non-NaN(s); expected exactly 1"
    ...
    def test_scenario_2c(self):
        ...
        assert len(set(dim_sizes)) == 1, "All dimensions should have the same number of coordinates"
        ...
        non_nan_indices = np.argwhere(non_nan_mask)  # shape (k, n) where n = number of dims
        ndim = dataarray.ndim
        ...
        for axis in range(ndim):
            axis_indices = non_nan_indices[:, axis]  # positional indices along this axis for all non-NaNs
    ...
    def test_scenario_3a(self):
        ...
        assert np.any(x0_with_data), "At least some x0 coordinates should have measurements"
        assert np.any(x1_with_data), "At least some x1 coordinates should have measurements"
    ...
    def test_scenario_3b(self):
        ...
        x0_with_data = np.any(~np.isnan(dataarray.values), axis=1)
        assert np.all(x0_with_data), "All x0 coordinates should have at least one measurement"
        # Check if all x1 coordinates are used
        x1_with_data = np.any(~np.isnan(dataarray.values), axis=0)
        assert np.all(x1_with_data), "All x1 coordinates should have at least one measurement"
