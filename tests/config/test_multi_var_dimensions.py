...
class TestFromListElement:
    ...
    def test_empty_list_raises_value_error(self):
        ...
        with pytest.raises(
            ValueError,
            match=r"Variable \d+ dim indices must have \d+ to \d+ elements, got \d+",
        ):
    ...
    def test_out_of_range_raises_value_error(self):
        ...
        with pytest.raises(
            ValueError, match=r"Variable \d+ dim indices .+ must be in range \[0, \d+\)"
        ):
    ...
    def test_duplicates_raise_value_error(self):
        ...
        with pytest.raises(
            ValueError, match=r"Variable \d+ dim indices .+ contain duplicates"
        ):
    ...
    def test_invalid_type_raises_type_error(self):
        ...
        with pytest.raises(
            TypeError, match=r"Variable \d+ var_dims must be int or list/tuple, got .+"
        ):
...
class TestFromList:
    ...
    def test_wrong_length_raises_value_error(self):
        ...
        with pytest.raises(
            ValueError, match=r"var_dims list must have \d+ elements, got \d+"
        ):
