...
class TestSelectRandomDims:
    ...
    def test_no_duplicates(self):
        ...
        result = MultiVarDimensionsConfig.select_random_dims(3, 5, 42)
    ...
    def test_reproducible_with_same_seed(self):
        ...
        result1 = MultiVarDimensionsConfig.select_random_dims(3, 5, rng1)
        result2 = MultiVarDimensionsConfig.select_random_dims(3, 5, rng2)
    ...
    def test_edge_case_select_one_dim(self):
        ...
        result = MultiVarDimensionsConfig.select_random_dims(1, 5, 42)
        assert len(result) == 1
        assert 0 <= result[0] < 5
...
class TestFromInt:
    ...
    def test_reproducible_with_seed(self):
        ...
        result1 = MultiVarDimensionsConfig.from_int(2, 3, 5, np.random.default_rng(123))
        result2 = MultiVarDimensionsConfig.from_int(2, 3, 5, np.random.default_rng(123))
        assert result1 == result2
...
class TestFromListElement:
    ...
    def test_negative_index_raises_value_error(self):
        ...
        with pytest.raises(ValueError, match="valid range"):
            MultiVarDimensionsConfig.from_list_element([0, -1], 5, 42)
    
    def test_invalid_type_raises_type_error(self):
        """Invalid type should raise TypeError."""
        with pytest.raises(TypeError, match="must be int, list, or tuple"):
            MultiVarDimensionsConfig.from_list_element("012", 5, 42)
    
    def test_result_sorted(self):
        """Result should be sorted."""
        result = MultiVarDimensionsConfig.from_list_element([2, 0, 1], 5, 42)
        assert result == [0, 1, 2]
...
class TestFromList:
    ...
    def test_all_integers(self):
        ...
        result = MultiVarDimensionsConfig.from_list([2, 3, 2], 5, 42)
        assert len(result) == 3
        for dims in result:
    ...
    def test_all_lists(self):
        ...
        result = MultiVarDimensionsConfig.from_list([[0, 1], [2, 3], [1, 4]], 5, 42)
        assert result == [[0, 1], [2, 3], [1, 4]]
    
    def test_mixed_integers_and_lists(self):
        """Mixed integers and lists should work."""
        result = MultiVarDimensionsConfig.from_list([2, [0, 1], 3], 5, 42)
        assert len(result) == 3
        assert result[1] == [0, 1]
    ...
    def test_wrong_length_raises_value_error(self):
        ...
        with pytest.raises(ValueError, match="must have length"):
            MultiVarDimensionsConfig.from_list([2, 3], 3, 42)
    
    def test_each_element_validated(self):
        """Each element should be validated."""
        with pytest.raises(ValueError, match="valid range"):
            MultiVarDimensionsConfig.from_list([[0, 10]], 5, 42)
