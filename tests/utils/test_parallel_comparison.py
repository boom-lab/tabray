...
def datasets_are_identical(ds1, ds2, variable="record"):
    ...
    if isinstance(ds1, xr.DataArray):
        if ds1.name != variable and variable != "record":
            return (
                False,
                f"DataArray name '{ds1.name}' doesn't match variable '{variable}'",
            )
        data1 = ds1.data
    else:
    ...
    if isinstance(ds2, xr.DataArray):
        if ds2.name != variable and variable != "record":
            return (
                False,
                f"DataArray name '{ds2.name}' doesn't match variable '{variable}'",
            )
        data2 = ds2.data
    else:
    ...
    nnan_count_1 = (
        (~da.isnan(data1)).sum().compute() if is_dask1 else (~np.isnan(data1)).sum()
    )
    nnan_count_2 = (
        (~da.isnan(data2)).sum().compute() if is_dask2 else (~np.isnan(data2)).sum()
    )

    if nan_count_1 != nan_count_2:
        return (
            False,
            f"Different number of NaNs in '{variable}' ({nan_count_1} vs {nan_count_2})",
        )
    if nnan_count_1 != nnan_count_2:
        return (
            False,
            f"Different number of non-NaNs in '{variable}' ({nnan_count_1} vs {nnan_count_2})",
        )

    return (
        True,
        "Datasets match all checked aspects (including counts of NaNs and non-NaNs).",
    )
...
def dataframes_coordinate_nan_match(df1, df2, coordinates, variable="record"):
    ...
    if other_coords:
        ...
        counts1 = (
            df1_grouped.groupby(other_coords + ["isnan"])
            .size()
            .reset_index(name="count")
        )
        counts2 = (
            df2_grouped.groupby(other_coords + ["isnan"])
            .size()
            .reset_index(name="count")
        )

        # Sort for comparison
        counts1 = counts1.sort_values(by=other_coords + ["isnan"]).reset_index(
            drop=True
        )
        counts2 = counts2.sort_values(by=other_coords + ["isnan"]).reset_index(
            drop=True
        )
        ...
        chunk_args.append(
            {
                ...
                "var_constant_coord_indices": copy.deepcopy(
                    gen.var_constant_coord_indices
                ),
...
class TestParallelSerialComparison:
    ...
    def test_scenario_1a_parallel_vs_serial(self, temp_dir):
        ...
        if pq_files:
            df_parallel = pd.concat(
                [pd.read_parquet(f) for f in pq_files], ignore_index=True
            )
        else:
    ...
    def test_multi_var_overlap_parallel_vs_serial(self, temp_dir):
        ...
        for var in ds_serial.data_vars:
            assert ds_parallel[var].dims == tuple(
                f"x{i}" for i in range(gen_parallel.num_dims)
            )
