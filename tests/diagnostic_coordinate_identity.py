...
def test_coordinate_arrays_match():
    ...
    print(f"Chunk 0 x1 first 5: {x1_arrays[0][:5]}")
    print(f"Chunk 0 x1 last 5: {x1_arrays[0][-5:]}")

    for i in range(1, len(chunks)):
        print(f"\nChunk {i} x1 shape: {x1_arrays[i].shape}")
        print(f"Chunk {i} x1 first 5: {x1_arrays[i][:5]}")
        print(f"Chunk {i} x1 last 5: {x1_arrays[i][-5:]}")
